...
export function SocialAuditHistory({ listLimit }: { listLimit?: number }) {
    const { user } = useUser();
    ...
    const historyQuery = useMemoFirebase(() => {
        ...
        let q = query(collection(firestore, 'users', user.uid, 'socialMediaAudits'), orderBy('timestamp', 'desc'));
    ...
    const { data: history, isLoading } = useCollection<SocialAuditHistoryItem>(historyQuery);
        ...
        return (
            <Card className="bg-gray-900/50 border-gray-800">
                <CardHeader>
                    ...
                    <CardDescription>Run your first social media audit to see your history here.</CardDescription>
                </CardHeader>
            </Card>
        );
    }
    ...
    return (
        <div className="space-y-4">
            {history.map(item => (
                <Link href={`/social-history/${item.id}`} key={item.id} className="block">

...
type SocialAuditResultDisplayProps = {
...
export function SocialAuditResultDisplay({ audit, url }: SocialAuditResultDisplayProps) {
  const { user } = useUser();
  ...
  async function onFollowUpSubmit(values: z.infer<typeof followUpFormSchema>) {
    ...
    try {
      const fullAnalysisContext: AuditResult = {
        summary: audit.summary,
        scores: {
          seo: audit.scores.profileOptimization,
          clarity: audit.scores.contentStrategy,
          cta: audit.scores.engagement,
          mobile: audit.scores.audienceGrowth,
        },
        strengths: audit.strengths,
        weaknesses: audit.weaknesses,
        blueprint: audit.blueprint,
        testing: audit.viralPostIdea,
      };

      const { answer } = await askFollowUp(fullAnalysisContext, values.question);
  ...
  const chartData = audit ? [
    { metric: "Profile", score: audit.scores.profileOptimization, fill: "hsl(var(--chart-1))" },
    { metric: "Content", score: audit.scores.contentStrategy, fill: "hsl(var(--chart-2))" },
    { metric: "Engagement", score: audit.scores.engagement, fill: "hsl(var(--chart-3))" },
    { metric: "Growth", score: audit.scores.audienceGrowth, fill: "hsl(var(--chart-4))" },
  ] : [];

  const chartConfig = {
    score: { label: "Score" },
    profile: { label: "Profile", color: "hsl(var(--chart-1))" },
    content: { label: "Content", color: "hsl(var(--chart-2))" },
    engagement: { label: "Engagement", color: "hsl(var(--chart-3))" },
    growth: { label: "Growth", color: "hsl(var(--chart-4))" },
  ...
  return (
    <div className="animate-in fade-in-50 duration-500 space-y-6">
        ...
        <Card className="bg-gray-900/50 border-gray-800">
         ...
         <Card className="bg-gray-900/50 border-gray-800">
            <CardHeader><CardTitle className="flex items-center gap-2"><Lightbulb className="text-yellow-400"/>Viral Post Idea</CardTitle></CardHeader>
            <CardContent><p className="text-gray-300">{audit.viralPostIdea}</p></CardContent>
        </Card>
                    ...
                    <form onSubmit={followUpForm.handleSubmit(onFollowUpSubmit)} className="mt-4 flex items-start gap-2 border-t border-gray-700 pt-4">
                        <FormField control={followUpForm.control} name="question" render={({ field }) => (
                            <FormItem className="flex-grow">
                                <FormControl>
                                    <Textarea
                                      placeholder="e.g., Can you give me three more post ideas?"
                                      {...field}
